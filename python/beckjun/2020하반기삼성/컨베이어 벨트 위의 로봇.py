...
def solution(n, k, container_belt):
    ...
    while k>0:
        ...
        if not visited[up_idx] and container_belt[up_idx]:
            ...
            visited[up_idx] = 1
            if not container_belt[up_idx]:
                k -= 1
        # 4
        res += 1
        if DEBUG: 
            print()
            print('robot_queue ',robot_queue)
            print('visited ', visited)
            print('container_belt ', container_belt)
            print('res: ',res)
            print("k: ", k)
            print("===============================")
    return res
if __name__ == "__main__":
    ...
    container_belt = [*map(int, input().split())]
    print(solution(n, k, container_belt))
